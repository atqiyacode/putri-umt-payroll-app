...
<template>
  <Head title="Group" />
  <TopBar name="Group" :canBack="true" :url="route('master.data')"></TopBar>
...
export default {
  ...
  methods: {
    ...
    saveData() {
      ...
      if (!this.v$.$error) {
        axios
          .post(route("group.store"), this.form)
          .then((result) => {
            this.$toast.add({
              severity: "success",
              summary: "Success",
    ...
    updateData() {
      axios
        .put(route("group.update", this.form.id), this.form)
        .then((result) => {
          this.$toast.add({
            severity: "success",
            summary: "Success",
